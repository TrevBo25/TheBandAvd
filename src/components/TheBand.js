...
export default class TheBand extends Component{
  ...
  toggleBand(bool){
    console.log('bool', bool, 'state', this.state.show);
    if(this.state.show && bool){
      console.log('hit');
      return;
    }
    this.setState((state) => ({show: bool}))
  }
  ...
  render(){
    return(
      <div className="bandholder">
        {this.state.show ? <div className="modalholder" ref={node => this.node = node}><BandInfo toggle={this.toggleBand} /></div> : null}
        ...
        {this.state.show ? <div className="bandbutton">LEARN LESS</div> : <div className="bandbutton" onClick={()=> this.toggleBand(true)}>LEARN MORE</div>}
      </div>
    )
  }
}